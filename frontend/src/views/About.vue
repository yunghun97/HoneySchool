  ...
  <div class="main_page container">
    ...
    <div class="row justify-content-center" style="height: 600px">
      ...
      <div class="col-5" id="createSession">
        ...
        <div v-else>
          <button class="btn-class" @click="joinSession">수업 참석하기</button>
        </div>
      </div>
      <!-- 선생님, 학생 분기 시간표 -->
      ...
      .then((response) => {
        ...
        const getThistime = () => {
          response.data.forEach(function (value: any, index: any, array: any) {
            ...
            // 수업시간 기준으로 교시 지정
            if (
              (startTimeH == hour && startTimeM <= minutes) ||
              (endTimeH == hour && endTimeM > minutes)
            ) {
            ...
            if (userinfo.position === "S") {
              router.push({
                ...
                params: {
                  ...
                  SessionId: mySessionName.value,
                  ClassName: thisClassName.value,
                },
              });
            } else {
              router.push({
