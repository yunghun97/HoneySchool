    ...
    <div class="submit-form">
        <form @submit.prevent="updateArticle">
            <div class="row mb-3">
                ...
                <div class="col-sm-10">
                <input class="form-control" id="title" required v-model="updatingarticle.title" type="text">
                </div>
            </div>
...
export default {
    ...
    setup() {
        ...
        const article = computed(() => store.state.boardStore.classBoardAll[id - 1]).value as BoardArticles;
        ...
        let updatingarticle = {
            ...
            file_link : article.file_link,
        } as BoardArticles
        const fileSelect = () => {
            const file = ref() as any
            console.log(file)
            updatingarticle.file_link= file.files;
        };
        const clearFiles = (event:any) => {
            event.preventDefault()
            updatingarticle.file_link=''
        };
        const updateArticle = () => {
            const formData = new FormData() 
            formData.append('category', updatingarticle.category);
            formData.append('title', updatingarticle.title);
            formData.append('content', updatingarticle.content);
            formData.append('file_link', updatingarticle.file_link);
        ...
        return { id, updatingarticle, isLoading, article, fileSelect, clearFiles, updateArticle}
    }
}
